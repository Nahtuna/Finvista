...
def _get_max_date_in_table(TableModel, symbol_col: str = "symbol", date_col: str = "date") -> Optional[str]:
    """Lấy giá trị MAX(date) trong bảng lịch sử giá (stock_history hoặc cw_history)."""
    db = SessionLocal()
    try:
        max_date = db.query(func.max(getattr(TableModel, date_col))).scalar()
        return max_date
    finally:
        db.close()
...
def check_data_freshness(expected_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    stock_latest = _get_max_date_in_table(StockHistoricalPrice)
...
def sync_atc_data(
    ...
    try:
        ...
        from src.infra.scraper_engine import ScraperEngine, DEFAULT_SEMAPHORE
