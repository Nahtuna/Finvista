...
from src.quant.engines.fetcher import fetch_market_cw_data
...
class VietstockScraper:
    ...
    def scrape_cw_page(self, cw_symbol, underlying_symbol, max_pages=3):
                ...
                for row in rows:
                    ...
                    if len(cols) >= 2:
                        ...
                        if link_el:
                            ...
                            link = "https://finance.vietstock.vn" + link_el['href'] if link_el['href'].startswith("/") else link_el['href']
                            if self._save_news(target_symbol, title, link, normalize_date(date_raw), category):
                                added_in_page += 1
                
                if added_in_page == 0 and page > 1: # No new items found (all already in DB)
                    break
