...
class SMCFeatures(Base):
    """Smart Money Concepts features for technical analysis."""
    ...
    # Pivot points (stored as JSON strings for SQLite compatibility)
    pivot_highs = Column(Text)  # JSON string of pivot high indices
    pivot_lows = Column(Text)   # JSON string of pivot low indices
    
    # Liquidity sweeps
    bsl_sweeps = Column(Text)   # JSON string of BSL sweep indices
    ssl_sweeps = Column(Text)   # JSON string of SSL sweep indices
    
    # Structure changes
    choch_bullish = Column(Text)  # JSON string of bullish CHoCH indices
    choch_bearish = Column(Text)  # JSON string of bearish CHoCH indices
    bos_bullish = Column(Text)    # JSON string of bullish BOS indices
    bos_bearish = Column(Text)    # JSON string of bearish BOS indices
    
    # Advanced patterns (to be added in Week 2)
    fvg = Column(Text)           # JSON string of FVG data
    order_blocks = Column(Text)  # JSON string of Order Block data
    wyckoff_events = Column(Text) # JSON string of Wyckoff event data
