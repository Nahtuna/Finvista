...
def get_udf_history(
    ...
    if is_cw_index:
        from backend.core.database import CWHistoricalPrice
    ...
    try:
        is_entrade_index = entrade_symbol in ["VNINDEX", "VN30", "HNX"]
        path_type = "index" if is_entrade_index else "stock"
        url = f"https://services.entrade.com.vn/chart-api/v2/ohlcs/{path_type}?resolution={entrade_res}&symbol={entrade_symbol}&from={from_time}&to={to_time}"
        ...
        import logging
        try:
            logging.warning(f"Entrade API offline/unreachable for {entrade_symbol} (falling back to database)")
        except (ValueError, IOError):
            # stderr closed, skip logging
            pass
    except Exception as e:
        # Use logging instead of print to avoid I/O errors
        import logging
        try:
            logging.warning(f"Entrade API error for {entrade_symbol}: {e}")
        except (ValueError, IOError):
            # stderr closed, skip logging
            pass
        pass
    ...
    if is_spx_index:
        try:
            ...
            import logging
            try:
                logging.warning(f"SPX fallback generation error: {e}")
            except (ValueError, IOError):
                # stderr closed, skip logging
                pass
            return {"s": "no_data", "t": [], "o": [], "h": [], "l": [], "c": [], "v": []}
    ...
    try:
        ...
        import logging
        try:
            logging.warning(f"[UDF] DB query error for {symbol_clean}: {db_err}")
        except (ValueError, IOError):
            # stderr closed, skip logging
            pass
        pass
    ...
    if not t_list:
        ...
        try:
            ...
            import logging
            try:
                logging.warning(f"[UDF] Fallback generation failed for {symbol_clean}: {fallback_err}")
            except (ValueError, IOError):
                # stderr closed, skip logging
                pass
