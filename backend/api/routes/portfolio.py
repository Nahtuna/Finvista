...
class RiskParams(BaseModel):
    stopLoss: float = 8.0
    takeProfit: float = 50.0
    maxPositionSize: float = 15.0
    maxPositions: int = 5
...
def run_longterm_backtest(
    ...
    stop_loss_pct: float = Query(15.0),
    take_profit_pct: float = Query(35.0),
    underlying_filter: str = Query("ALL"),
):
    """
    Long-term historical backtest using SQLite database with multi-year analysis.
    """
    return execute_db_backtest(
        strategy=strategy,
        years=years,
...
def save_custom_strategy(
    ...
    """
    Save a custom backtest strategy to the database.
    """
    import sqlite3
    import os
    import json
    ...
    try:
        ...
        if strategy.id:
            ...
            cursor.execute("""
                ...
                SET name = ?, entry_conditions = ?, exit_conditions = ?, 
                    risk_params = ?, updated_at = ?
                WHERE id = ? AND username = ?
            """, (
                strategy.name,
                json.dumps([c.dict() for c in strategy.entryConditions]),
                json.dumps([c.dict() for c in strategy.exitConditions]),
                json.dumps(strategy.riskParams.dict()),
                now,
                strategy.id,
            ...
            cursor.execute("""
                ...
                (username, name, entry_conditions, exit_conditions, risk_params, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (
                current_user["username"],
                strategy.name,
...
def list_custom_strategies(current_user: dict = Depends(get_current_user)):
    """
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id, name, entry_conditions, exit_conditions, risk_params, created_at, updated_at
        ...
        for row in cursor.fetchall():
            strategies.append({
                "id": row[0],
                "name": row[1],
                "entryConditions": json.loads(row[2]),
                "exitConditions": json.loads(row[3]),
                "riskParams": json.loads(row[4]),
                "createdAt": row[5],
                "updatedAt": row[6]
            })
...
def delete_custom_strategy(
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE id = ? AND username = ?
        """, (strategy_id, current_user["username"]))
...
def run_custom_backtest(
    ...
    """
    Run backtest using a custom strategy with user-defined entry/exit conditions.
    This endpoint interprets the visual strategy builder conditions and executes them.
    """
    # For now, return a placeholder response
    # In a full implementation, this would parse the conditions and execute custom logic
    return {
        "status": "success",
        "message": "Custom backtest execution initiated",
        "strategy": strategy.name,
        "note": "Full custom strategy execution engine to be implemented"
    }
