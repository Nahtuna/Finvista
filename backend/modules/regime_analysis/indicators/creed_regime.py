...
def calculate_creed_regime_from_df(df: pd.DataFrame, trend_period: int = 200) -> Dict[str, Any]:
    ...
    # Calculate RSI 14 to filter whipsaws and capture fast reversals
    delta = pd.Series(closes).diff()
    gain = delta.clip(lower=0).rolling(window=14, min_periods=1).mean()
    loss = (-delta.clip(upper=0)).rolling(window=14, min_periods=1).mean()
    rs = gain / np.where(loss == 0, 1e-9, loss)
    rsi14 = 100 - (100 / (1 + rs))
    latest_rsi = rsi14.values[-1] if len(rsi14) > 0 else 50.0
    ...
    # Fast momentum check for early reversal detection
    fast_reversal_bear = (latest_close < latest_ema20) and (latest_rsi < 42)
    fast_reversal_bull = (latest_close > latest_ema20) and (latest_rsi > 58)

    # Determine Phase using volatility and momentum filters
    if (momentum_bull and dist_pct > 0.005) or fast_reversal_bull:
    ...
    elif momentum_bear or dist_pct < -0.005 or fast_reversal_bear:
