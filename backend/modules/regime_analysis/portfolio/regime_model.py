...
class HybridRegimeModel:
    ...
    def predict(self, X) -> np.ndarray:
        if isinstance(X, pd.DataFrame) and 'trend' in X.columns:
            trends = X['trend'].values.astype(int)
            from sklearn.preprocessing import StandardScaler
            X_raw = X[['log_return', 'rolling_vol', 'log_volume_ratio']].values
            X_scaled = self.scaler.transform(X_raw)
            hmm_states = self.hmm_model.predict(X_scaled)
            return 2 * (1 - trends) + hmm_states
    ...
    def predict_proba(self, X) -> np.ndarray:
        if isinstance(X, pd.DataFrame) and 'trend' in X.columns:
            ...
            for t in range(len(X)):
                ...
                if tr == 1:
                    probs[t] = [p_low, p_high, 0.0, 0.0]
                else:
                    probs[t] = [0.0, 0.0, p_low, p_high]
            return probs
        if len(X) == len(self.train_probs):
            return self.train_probs
        return self.train_probs[-len(X):]
...
def prepare_vnindex_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Accurate Trend Filter:
    # Bullish requires: Close > SMA20 AND Close > SMA50 AND Volume >= 0.9 * Vol_MA20
    # Technical pullbacks in downtrends (Close < SMA50 or Vol low) are marked as Bearish (0)
    is_price_bull = (df['close'] > df['sma_20']) & (df['close'] > df['sma_50'])
    is_vol_confirmed = df['volume'] >= (df['vol_ma20'] * 0.90)
    df['trend'] = (is_price_bull & is_vol_confirmed).astype(int)
