...
from backend.modules.regime_analysis.indicators.volatility_forecasting import get_underlying_garch_evt_var, get_conformal_calibrated_var
from backend.modules.cw_pricing.models.pricing_core import calculate_merton_jump_diffusion_price

# Default file paths
PORTFOLIO_FILE = os.path.join("data", "config", "paper_portfolio.json")
# REPORT_PATH removed - using database only
...
def load_portfolio(username: str = "demo") -> dict:
    ...
    from backend.core.database import SessionLocal, User, Portfolio, Position, TransactionHistory
...
def log_nav_checkpoint(db, user_id: int):
    ...
    from backend.core.database import Portfolio, Position, MarketOpportunity, PortfolioNavHistory
...
def execute_buy(symbol: str, underlying: str, price: float, score: float, days_left: int, username: str = "demo", mkt_regime: dict = None) -> dict:
    ...
    from backend.infra.orderbook_scraper import get_real_order_book, calculate_slippage
...
def scan_and_trade(force: bool = False, username: str = "demo", approved_symbols: list = None) -> list:
    ...
    # ── Step 0: Detect Market Regime (Ensemble) ───────────────────────────
    ...
    try:
        ...
        # Try to get VNINDEX data for ensemble calculation
        ...
        if not df_regime.empty and len(df_regime) >= 50:
            ...
            mkt_regime = ensemble.calculate_ensemble_regime(df_regime, "VNINDEX")
            regime_name = mkt_regime.get("regime", "UNKNOWN")
            regime_bias = mkt_regime.get("bias", "LONG_CW")
            regime_source = f"Ensemble ({mkt_regime.get('performance_mode', 'HYBRID')})"
            print(f"🌐 MARKET REGIME DETECTED: {regime_name} ({regime_bias}) - Source: {regime_source}")
        else:
            # Fallback to HMM if insufficient data
            ...
            regime_name = mkt_regime.get("regime", "UNKNOWN")
            regime_bias = mkt_regime.get("bias", "LONG_CW")
            print(f"🌐 MARKET REGIME DETECTED (HMM Fallback): {regime_name} ({regime_bias})")
    except Exception as e:
    ...
    for symbol in held_symbols:
        ...
        ai_force_sell = False
        try:
            from backend.core.database import SessionLocal as _SL, CompanyDistressAnalysis as _CDA
            _db3 = _SL()
            try:
                _health = (
                    _db3.query(_CDA)
        ...
        try:
            from backend.modules.regime_analysis.indicators.garch_evt_var import get_conformal_calibrated_var
