...
from backend.core.database import SessionLocal, User, Portfolio, Position, TransactionHistory, MarketOpportunity
from backend.modules.trading_engine.paper_trader import (
    INITIAL_CASH, BUY_FEE_RATE, SELL_FEE_RATE, HOSE_LOT_SIZE, 
    MAX_ALLOCATION_PCT, MATURITY_LIMIT_DAYS,
    calculate_settlement_date, is_market_open
)
...
class PortfolioService:
    ...
    def get_portfolio(username: str) -> Dict[str, Any]:
        ...
        try:
            ...
            from backend.core.database import PortfolioNavHistory
    ...
    def place_order(username: str, symbol: str, side: str, qty: Optional[int] = None, price_override: Optional[float] = None, reason: str = "Manual Order") -> Dict[str, Any]:
        ...
        from backend.modules.trading_engine.paper_trader import execute_buy, execute_sell
        ...
        try:
            ...
            if side_clean == "BUY":
                ...
                if qty is None:
                    ...
                    from backend.modules.trading_engine.paper_trader import log_nav_checkpoint
    ...
    def reset_portfolio(username: str) -> Dict[str, Any]:
        ...
        from backend.modules.trading_engine.paper_trader import reset_portfolio as legacy_reset
    ...
    def scan_and_trade(username: str, force: bool = False) -> List[str]:
        ...
        from backend.modules.trading_engine.paper_trader import scan_and_trade as legacy_scan
    ...
    def get_risk_report(username: str) -> Dict[str, Any]:
        ...
        try:
            ...
            from backend.modules.cw_pricing.backtest.risk_engine import MarketRiskEngine
