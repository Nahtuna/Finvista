...
from backend.infra.chart_generator import generate_candlestick_base64
...
from backend.core.database import SessionLocal, MarketOpportunity, AIAnalysisMemory, CorporateNews, CorporateEvent
...
class AICommitteeService:
    ...
    def __init__(self):
        self.ai_client = get_ai_client(module_name="ai_committee", auto_start_proxy=False)
...
- Trả lời ngắn gọn, chuyên nghiệp bằng tiếng Việt."""
    ...
    async def _layer5_volatility_arb(self, symbol: str, quant_data: Dict[str, Any]) -> str:
        """Layer 5: Options Math & Volatility Forensic Specialist (Rule-based)."""
        try:
            from backend.modules.trading_engine.rule_based_volatility import get_volatility_analyzer
            analyzer = get_volatility_analyzer()
            return analyzer.analyze_volatility(symbol, quant_data)
        except Exception as e:
            # Fallback đơn giản nếu rule-based lỗi
            iv = quant_data.get('iv', 30.0)
            hv = quant_data.get('hv', 25.0)
            delta = quant_data.get('delta', 0.5)
            days = quant_data.get('days_to_maturity', 90)
            
            iv_hv_ratio = iv / hv if hv > 0 else 1.0
            return f"Volatility Analysis: IV {iv}% vs HV {hv}% (ratio {iv_hv_ratio:.1f}x). Delta {delta:.2f}, Còn {days} ngày."
