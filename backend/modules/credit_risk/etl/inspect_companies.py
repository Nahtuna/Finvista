...
import os
import pandas as pd
from backend.core.utils import logger, load_json, save_json
...
def get_companies_data() -> pd.DataFrame:
    ...
    try:
        ...
        save_json(records, config.COMPANIES_LIST_FILE)
        logger.info(f"💾 Company list cached to: {cache_file} and {config.COMPANIES_LIST_FILE}")
        return df
