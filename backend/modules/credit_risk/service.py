...
class CreditRiskService:
    ...
    def _get_ai_commentary(ticker: str, latest: pd.Series, z_score: float) -> Optional[str]:
        """Generate commentary based purely on financial metrics (không phụ thuộc AI)."""
        try:
            ...
            # Xác định vùng rủi ro dựa trên Altman Z-Score
            if z_score < 1.1:
                risk_zone = "Vùng Nguy Hiểm (Danger Zone) - Rủi ro kiệt quệ tài chính cao"
            elif z_score <= 2.6:
                risk_zone = "Vùng Cảnh Báo (Grey Zone) - Sức khỏe tài chính trung bình"
            else:
                risk_zone = "Vùng An Toàn (Green Zone) - Nền tảng tài chính lành mạnh"
            
            # Build commentary dựa trên số liệu
            commentary_parts = [
                f"Doanh nghiệp {ticker} hiện {risk_zone} (Altman Z-Score: {z_score:.2f})."
            ]
            
            # Cơ cấu vốn
            if debt_ratio > 0.6:
                commentary_parts.append(f"Tỷ lệ nợ cao ({debt_ratio*100:.1f}%), tạo áp lực chi phí lãi vay.")
            elif debt_ratio < 0.3:
                commentary_parts.append(f"Cơ cấu vốn an toàn với tỷ lệ nợ thấp ({debt_ratio*100:.1f}%).")
            else:
                commentary_parts.append(f"Cơ cấu vốn cân đối với tỷ lệ nợ {debt_ratio*100:.1f}%.")
            
            # Khả năng thanh toán
            if current_ratio < 1.0:
                commentary_parts.append(f"Khả năng thanh toán ngắn hạn yếu (Current Ratio: {current_ratio:.2f}).")
            elif current_ratio > 2.0:
                commentary_parts.append(f"Khả năng thanh toán ngắn hạn mạnh (Current Ratio: {current_ratio:.2f}).")
            else:
                commentary_parts.append(f"Khả năng thanh toán ngắn hạn đạt yêu cầu (Current Ratio: {current_ratio:.2f}).")
            
            # Hiệu quả kinh doanh
            if profit_after_tax < 0:
                commentary_parts.append(f"Hoạt động kinh doanh gặp khó khăn với lỗ ròng {abs(profit_after_tax)/1e9:.1f} tỷ VND.")
            else:
                commentary_parts.append(f"Hoạt động kinh doanh có lãi sau thuế {profit_after_tax/1e9:.1f} tỷ VND.")
            
            # Dòng tiền
            if operating_cash_flow < 0:
                commentary_parts.append(f"Dòng tiền hoạt động kinh doanh âm ({operating_cash_flow/1e9:.1f} tỷ VND), cần chú ý thanh khoản.")
            else:
                commentary_parts.append(f"Dòng tiền hoạt động kinh doanh dương ({operating_cash_flow/1e9:.1f} tỷ VND).")
            
            # Khả năng trả lãi
            if ebit_to_interest < 2.0:
                commentary_parts.append(f"Khả năng trả lãi yếu (EBIT/Interest: {ebit_to_interest:.1f}x).")
            elif ebit_to_interest > 5.0:
                commentary_parts.append(f"Khả năng trả lãi mạnh (EBIT/Interest: {ebit_to_interest:.1f}x).")
            
            return " ".join(commentary_parts)
            
        except Exception as e:
            print(f"⚠️ Commentary generation failed: {e}")
            return f"{ticker} - Altman Z-Score: {z_score:.2f}. Không có chi tiết do lỗi xử lý số liệu."
