...
import numpy as np
import joblib
from backend.core.utils import logger, load_csv, save_csv
...
def evaluate_market_health():
    ...
    # 4. Prepare features for ML Model using scaler's fitted feature names
    feature_names_path = config.FEATURE_NAMES_ARTIFACT
    if hasattr(scaler, "feature_names_in_"):
        feature_cols = list(scaler.feature_names_in_)
        logger.info(f"📋 Loaded {len(feature_cols)} feature names directly from scaler artifact.")
    elif os.path.exists(feature_names_path):
        feature_cols = joblib.load(feature_names_path)
        logger.info(f"📋 Loaded {len(feature_cols)} feature names from {feature_names_path}")
    else:
        exclude_cols = {
        ...
        logger.warning("⚠️ Saved feature names not found. Using fallback dynamic columns.")
    
    # Ensure all required features are present in the dataframe
    for col in feature_cols:
        if col not in latest_records.columns:
            latest_records[col] = 0.0
