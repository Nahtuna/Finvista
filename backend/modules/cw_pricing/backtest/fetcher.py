...
def fetch_market_cw_data(bypass_cache: bool = False) -> pd.DataFrame:
    ...
    try:
        from backend.infra.market_cache import should_use_cache, load_snapshot, save_snapshot, get_session_status
        sess = get_session_status()
        status_str = str(sess.get('status', ''))
        try:
            safe_print(f"   [SESSION] Session status: {status_str}")
        except Exception:
            try:
                clean_status = status_str.encode('ascii', 'replace').decode('ascii')
                safe_print(f"   [SESSION] Session status: {clean_status}")
            except Exception:
                pass
        if sess["use_cache"] and not bypass_cache:
            cached_df = load_snapshot()
            if cached_df is not None and not cached_df.empty:
                safe_print("   [CACHE] Loaded data from session cache (outside trading hours).")
                return cached_df
            else:
                safe_print("   [CACHE] No valid cache, continuing to fetch live...")
        _market_cache_module = (should_use_cache, save_snapshot)
    except ImportError:
        _market_cache_module = None
    
    safe_print("Ingesting live Covered Warrant data from trading APIs...")
    headers = {
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 200:
                ...
                safe_print(f"[WARNING] Primary symbol registry returned {resp.status_code}. Retrying in {backoff}s... (Attempt {attempt+1}/{max_retries})")
                import time
                time.sleep(backoff)
                backoff *= 2
        except Exception:
    ...
    df = pd.DataFrame()
    if symbols:
        safe_print(f"[FETCH] Loading market details for {len(symbols)} warrant instruments...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if resp_details.status_code == 200:
                    ...
                    safe_print(f"[WARNING] Market details API returned {resp_details.status_code}. Retrying in {backoff}s... (Attempt {attempt+1}/{max_retries})")
                    import time
                    time.sleep(backoff)
                    backoff *= 2
            except Exception:
        ...
        if resp_details and resp_details.status_code == 200:
            ...
            try:
                ...
                try:
                    ...
                    safe_print(f"[WARNING] Failed to query historical CW turnover: {dbe}")
                finally:
                    db.close()
            except Exception as e:
                safe_print(f"[WARNING] Database connection error for historical CW turnover: {e}")
                
            df['hist_avg_gtgd'] = df['A_MaCW'].map(hist_avg_gtgd_map).fillna(0.0)

    # Fallback to SQLite DB if live fetch failed completely
    if df.empty:
        safe_print("\n[WARNING] Live trading API is temporarily down (503 Service Unavailable).")
        safe_print("🚀 Activating SQLite DB offline cache fallback...")
        try:
            ...
            if not df.empty:
                safe_print(f"💡 Successfully loaded {len(df)} warrants from DB cache!")
                safe_print("🖥️ Terminal running in OFFLINE mode with full pricing & Greeks simulator enabled!\n")
            ...
            safe_print(f"❌ DB cache fallback failed: {e}")
        safe_print("❌ Live API down and DB cache is empty. Cannot continue.")
        return pd.DataFrame()

    # Step 3: Batch-fetch target underlying stock quotes (only if live succeeded)
    stocks = df['B_MaCPCS'].dropna().unique().tolist()
    if stocks:
        safe_print(f"[FETCH] Mapping last traded prices for {len(stocks)} underlying assets...")
        url_details = "https://trading.vietcap.com.vn/api/price/symbols/getList"
        resp_stocks = None
        for attempt in range(max_retries):
            try:
        ...
        try:
            ...
            safe_print(f"   ⚠️ Database stock prices fallback query failed: {dbe}")
        ...
        if resp_stocks and resp_stocks.status_code == 200:
            try:
                ...
                safe_print(f"   ⚠️ Failed to parse API stock prices: {e}")
        else:
            safe_print("⚠️ Failed underlying mapping API call. Reverting to database/reference prices.")
    ...
    try:
        from backend.core.database import SessionLocal, CompanyDistressAnalysis
        db = SessionLocal()
        try:
            ...
            if records:
                ...
                safe_print(f"📊 Integrated live ML credit risk check from SQLite! Dynamic distress mapping complete for {len(prob_map)} underlying companies.")
            else:
                df['O_Stock_FA'] = 18.5
            ...
            safe_print(f"⚠️ SQLite query for credit health mapping failed: {e}. Reverting to defaults.")
            df['O_Stock_FA'] = 18.5
            df['underlying_is_distressed'] = 0
            df['underlying_distress_prob'] = 0.10
            df['underlying_altman_z'] = 3.0
    ...
    try:
        ...
        try:
            ...
            if sys_records:
                ...
                safe_print(f"🕸️ DebtRank systemic risk integrated from DB! {count_sys} CWs in systemic danger zone.")
            else:
                ...
                safe_print("⚠️ No systemic data in DB — run `python run.py credit --contagion` to populate it.")
        finally:
            db.close()
    except Exception as e:
        df['underlying_systemic_prob'] = 0.10
        ...
        df['underlying_systemic_status'] = 'GREEN (SAFE)'
        df['underlying_systemic_is_distressed'] = 0
        safe_print(f"[WARNING] DebtRank layer failed gracefully: {e}")

    # ── Lưu cache sau khi fetch live thành công ──
    try:
        from backend.infra.market_cache import save_snapshot, is_trading_session
...
def fetch_underlying_historical_volatilities(symbols: list) -> dict:
    ...
    Prioritizes the stock_history table in SQLite DB to avoid vnstock rate limit blocks,
    falling back to dynamic API fetch if needed.
    """
    hv_map = {}
    if not symbols:
        return hv_map
    ...
    safe_print("\n" + "=" * 85)
    safe_print(f"📡 Resolving Multi-Window Historical Volatility (10N, 20N, 40N) for {len(symbols)} assets...")
    safe_print("=" * 85)
    ...
    try:
        ...
        if not local_df.empty:
            ...
            safe_print(f"   💾 Loaded stock history from DB: {len(local_df)} rows. Analyzing volatility profiles...")
    except Exception as e:
        safe_print(f"   ⚠️ Failed to load stock history from DB ({e}). Reverting to dynamic endpoints.")
    ...
    for sym in symbols:
        ...
        if not local_df.empty and 'symbol' in local_df.columns:
            ...
            if len(sym_data) >= 15:
                try:
                    ...
                    safe_print(f"   📊 {sym:<8} -> HV10: {hv_10*100:5.1f}% | HV20: {hv_20*100:5.1f}% | HV40: {hv_40*100:5.1f}% (Local Cache)")
                    continue
                except Exception:
                    pass
            ...
            try:
                ...
                if not df.empty and 'close' in df.columns:
                    ...
                    if len(log_ret) >= 5:
                        ...
                        safe_print(f"   📡 {sym:<8} -> HV10: {hv_10*100:5.1f}% | HV20: {hv_20*100:5.1f}% | HV40: {hv_40*100:5.1f}% (Live Fetch)")
        ...
        if not fetched_success:
            ...
            safe_print(f"   ⚠️ {sym:<8} -> HV10: {def_val*100:5.1f}% | HV20: {def_val*100:5.1f}% | HV40: {def_val*100:5.1f}% (Fallback Default)")
...
def fetch_derivatives_sentiment() -> dict:
    ...
    try:
        ...
        import traceback
        safe_print(f"   ⚠️ [fetch_derivatives_sentiment] Error: {type(e).__name__}: {e}")
        try:
            traceback.print_exc()
        except (ValueError, OSError):
            pass
