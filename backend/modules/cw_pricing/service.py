...
from backend.core.database import MarketOpportunity, SessionLocal, CorporateNews, CorporateEvent
...
from backend.infra.trade_scraper import get_ssi_trades, reconstruct_cvd
...
def _fetch_vps_index_live(symbol: str) -> dict | None:
    ...
    try:
        ...
        if d.get("s") == "ok" and d.get("c") and len(d["c"]) >= 2:
            ...
            # Safe-check: validate values are reasonable
            if c0 <= 0 or c1 <= 0:
                return None
            # Apply scaling for HNXINDEX if value is too large
            if symbol == "HNXINDEX" and c0 > 10000.0:
                c0 /= 1000.0
                c1 /= 1000.0
            chg = round(c0 - c1, 2)
            pct = round((chg / c1 * 100) if c1 > 0 else 0.0, 2)
            # Validation: cap unreasonable percentage changes
            if abs(pct) > 1000.0:
                pct = 0.0
                chg = 0.0
...
class WarrantService:
    ...
    def _is_opportunities_stale() -> bool:
        ...
        """
        import json, os
        from backend.infra.market_cache import CACHE_FILE
        try:
    ...
    def _bg_refresh_pipeline(strategy: str = "balanced") -> None:
        ...
        try:
            ...
            # Safe logging for background threads - use NullHandler if stderr is closed
            try:
                import logging
                ...
                logger.warning(f"[WarrantService] Background refresh error: {e}")
            except (ValueError, IOError):
                # stderr closed, skip logging
                pass
        finally:
        ...
        try:
            ...
            if count == 0 or force_refresh:
                ...
                try:
                    ...
                    logger = get_logger("financial_distress")
                    try:
                        logger.warning(f"[WarrantService] Blocking pipeline refresh failed: {pe}. Attempting offline CSV recovery.")
                    except (ValueError, IOError):
                        # stderr closed, skip logging
                        pass
                    try:
                        from backend.modules.cw_pricing.backtest.reporter import load_opportunities_from_db, save_opportunities_to_db
                        # Fallback to load from CSV
                        fallback_df = load_opportunities_from_db(fallback_to_csv=True)
                        if not fallback_df.empty:
                            save_opportunities_to_db(fallback_df)
                            try:
                                logger.info("[WarrantService] Successfully recovered and populated DB from offline CSV fallback.")
                            except (ValueError, IOError):
                                # stderr closed, skip logging
                                pass
                    except Exception as fe:
                        try:
                            logger.warning(f"[WarrantService] Offline recovery failed: {fe}")
                        except (ValueError, IOError):
                            # stderr closed, skip logging
                            pass
                ...
                import logging
                try:
                    logging.info("[WarrantService] 🔄 Stale data detected — background refresh triggered.")
                except (ValueError, IOError):
                    # stderr closed, skip logging
                    pass
            ...
            for row in opps_list:
                results.append({
                    ...
                    "implied_volatility_pct": (
                        round(float(row.implied_volatility_pct), 2)
    ...
    def get_market_metadata(force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Get market status
                try:
                    from backend.infra.market_cache import get_session_status
    ...
    def get_underlyings(news_limit: int = 20, language: str = "en", force_refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                indices_dict = {}
                # Define fallback_map before the loop so it's accessible inside
                fallback_map = {
                ...
                for idx_sym in ["VNINDEX", "VN30", "HNXINDEX", "UPCOM", "SPX", "NDX"]:
                    try:
                        # Get the 2 most recent records with dates
                        rows = db.execute(text(
                            "SELECT date, close FROM stock_history WHERE symbol = :sym AND close > 0 ORDER BY date DESC LIMIT 2"
                        ), {"sym": idx_sym}).fetchall()
                        
                        if rows and len(rows) > 0 and rows[0][1]:
                            current_date = rows[0][0]
                            close_v = float(rows[0][1])
                            prev_v = float(rows[1][1]) if len(rows) > 1 and rows[1][1] else close_v
                            prev_date = rows[1][0] if len(rows) > 1 else current_date
                            
                            # Smart scaling: detect and handle corrupted data patterns
                            # Pattern 1: Both values are large (>10000) - scale both
                            # Pattern 2: Only current is large, previous is normal - scale only current
                            # Pattern 3: Both are normal - no scaling needed
                            if idx_sym in ["VNINDEX", "VN30", "HNXINDEX"]:
                                current_scaled = False
                                prev_scaled = False
                                ...
                                # Additional check: if current was scaled but previous wasn't, 
                                # and the ratio is still extreme, it might be a data corruption issue
                                if current_scaled and not prev_scaled and prev_v > 0:
                                    ratio = close_v / prev_v
                                    if ratio > 100 or ratio < 0.01:
                                        # Use fallback from live API or fallback_map
                                        c, ch, p = fallback_map[idx_sym]
                                        indices_dict[idx_sym] = {"close": c, "change": ch, "pct": p}
                                        continue
                            
                            # Additional validation: check if prev_v is unreasonably different from close_v
                            # If prev_v is too small relative to close_v, it's likely data corruption
                            if prev_v > 0 and (close_v / prev_v > 100 or prev_v / close_v > 100):
                                c, ch, p = fallback_map[idx_sym]
                                indices_dict[idx_sym] = {"close": c, "change": ch, "pct": p}
                                continue
                            
                            # Safe-check: ensure prev_v is valid and not zero
                            if prev_v is None or prev_v == 0 or not isinstance(prev_v, (int, float)):
                                prev_v = close_v  # Fallback to current price if prev is invalid
                            
                            change_v = close_v - prev_v
                            pct_v = (change_v / prev_v * 100.0) if prev_v > 0 else 0.0
                            
                            # Validation: cap unreasonable percentage changes (likely data errors)
                            if abs(pct_v) > 1000.0:  # More than 1000% change is likely an error
                                c, ch, p = fallback_map[idx_sym]
                                indices_dict[idx_sym] = {"close": c, "change": ch, "pct": p}
                                continue
