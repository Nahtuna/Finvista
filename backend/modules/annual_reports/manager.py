...
from backend.modules.annual_reports.scraper import CafeFReportScraper
...
class AnnualReportManager:
    ...
    def list_available_reports(self, ticker: str) -> List[Dict[str, Any]]:
        """Lists all reports available for a ticker from CafeF local cache."""
        reports = []
        ticker_upper = ticker.upper()

        # Look up in CafeF downloaded folder
    ...
    def get_report_pdf_bytes(self, ticker: str, year: int, quarter: int = 5) -> Optional[bytes]:
        """Gets PDF file bytes from CafeF local cache directory."""
        ticker_upper = ticker.upper()

        # Check in CafeF local cache directory
        ticker_local_dir = LOCAL_PDF_DIR / ticker_upper
        if ticker_local_dir.exists():
    ...
    def query_report(self, ticker: str, year: int, quarter: int, question: str) -> str:
        ...
        ai_client = get_ai_client(module_name="annual_reports", auto_start_proxy=False)
